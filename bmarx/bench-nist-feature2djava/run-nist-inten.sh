...
ODIR="/data/OUT-nistjavaplugin-inten"
...
FEATURES="Feature2DJava_Mean,Feature2DJava_Entropy,Feature2DJava_Median,Feature2DJava_Mode,Feature2DJava_StandardDeviation,Feature2DJava_Skewness,Feature2DJava_Kurtosis,Feature2DJava_Hyperskewness,Feature2DJava_Hyperflatness"
docker run -it --mount type=bind,source=/home/kharchenkoa2/work/data,target=/data 55cc0153ebb1 --imageDir $IDIR --partitionsDir $SDIR --iP ".*" --pP ".*" --outputDir $ODIR --features $FEATURES

end=$(date +%s)
echo "Elapsed Time: $(($end-$start)) seconds"
echo "***" >> timing_inten.txt
date >> timing_inten.txt
echo $IDIR >> timing_inten.txt
echo $SDIR >> timing_inten.txt
echo $FEATURES >> timing_inten.txt
